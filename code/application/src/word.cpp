...
static int fb_dev_init(void)
{
    ...
    /* 内存映射 */
    screen_base = (unsigned short *)mmap(NULL, screen_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
            ...
            const wchar_t *str, unsigned int color)
{
    ...
    long int temp;
    unsigned int width = fb_fix.line_length / (fb_var.bits_per_pixel / 8);
    int n;
    int i, j, p, q;
...
int show_print()
{
    ...
    // std::cout << "123"  << "\n";
    /* freetype初始化 */
    // if ("/usr/share/fonts/ttf/SourceHanSansCN-Regular.otf", 0)
    if (freetype_init(std::string("/usr/share/fonts/ttf/Nice Postman.otf").c_str(), 0))
        exit(EXIT_FAILURE);
    // std::cout << "456"  << "\n";

    /* 在LCD上显示中文 */
    int y = fb_var.yres * 0.25;
